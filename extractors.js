...
class EntityExtractor {
    async extractEntity (message) {
        const groq = new Groq({ apiKey: process.env.GROQ_API_KEY});
        let chatCompletion = await groq.chat.completions.create({
            messages: [
                {
                    role: "system",
                    content: "You will be given a weather inquiry and you should extract the city from it. So, in the inquiry 'Weather in Cairo', your response should be 'Cairo'. If the inquiry was about a county, respond with the capital of that country. If the city is not recognized, respond with 'undefined'. If no city was provided, respond with 'local'. The extracted city should be translated to english. If only some place was mentioned, respond with the city where is exists",
                },
                {
                    role: "user",
                    content: `Weather inquiry: ${message}`,
                },
            ],
